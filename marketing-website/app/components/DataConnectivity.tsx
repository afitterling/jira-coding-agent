...
/** Connector chips are proper nouns — they stay literal, matched to copy by index. */
const CONNECTOR_CHIPS: string[][] = [
  ["Postgres", "AWS DynamoDB", "MySQL", "MongoDB", "Snowflake", "BigQuery"],
  ["REST", "GraphQL", "Jira", "Slack"],
  ["AWS S3", "Nextcloud", "Azure Blob", "CSV", "Parquet"],
  ["Kafka", "Queues", "Market data", "Alt-data"],
];

export function DataConnectivity() {
  const { t } = useT();
  const dc = t.dataConnectivity;
  const connectors = dc.connectors.map((c, i) => ({ ...c, chips: CONNECTOR_CHIPS[i] }));
  return (
    <section
      id="data"
        ...
        <div className="max-w-3xl">
          ...
          <Reveal delay={80}>
            <h2 className="mt-6 text-balance text-4xl font-extrabold leading-[1.08] tracking-tight text-white sm:text-5xl lg:text-6xl">
              {dc.titleLead}<span className="text-gradient">{dc.titleAccent}</span>
            </h2>
          </Reveal>
          <Reveal delay={140}>
            <p className="mt-5 max-w-2xl text-pretty text-lg leading-relaxed text-slate-400">
              {dc.introP1}<span className="font-medium text-indigo-200">{dc.introName}</span>
              {dc.introP2}
            </p>
          </Reveal>
        </div>
            ...
            <div className="flex min-w-max items-center gap-2 font-mono text-xs">
              {dc.pipeline.map((step, i) => (
                <div key={step} className="flex items-center gap-2">
                  ...
                  {i < dc.pipeline.length - 1 && <span className="text-accent/60">→</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <Reveal delay={80}>
          <div className="mt-12 flex items-center gap-3">
            ...
            <span className="rounded-full border border-accent-cyan/30 bg-accent-cyan/10 px-2.5 py-0.5 font-mono text-xs font-semibold text-cyan-200">
              {dc.outputs.length} {dc.outputsCount}
            </span>
          </div>
        </Reveal>
